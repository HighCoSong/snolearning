...
const DEPT_BY_COLLEGE: { college: string; depts: string[] }[] = [
  ...
  {
    college: "생활과학대학",
    depts: ["가족자원경영학과", "아동복지학부", "의류학과", "식품영양학과"],
  },
  {
    college: "사회과학대학",
    depts: [
...
const CATEGORIES = ["장학금", "취업/인턴십", "공모전", "행사/프로그램", "기타"];
...
function categorize(item: NoticeItem): string {
  ...
  if (CATEGORY_KEYS["장학금"].some((kw) => text.includes(kw.toLowerCase())))
    return "장학금";
  if (
    CATEGORY_KEYS["취업/인턴십"].some((kw) => text.includes(kw.toLowerCase()))
  )
    return "취업/인턴십";
  if (CATEGORY_KEYS["공모전"].some((kw) => text.includes(kw.toLowerCase())))
    return "공모전";
  if (
    CATEGORY_KEYS["행사/프로그램"].some((kw) => text.includes(kw.toLowerCase()))
  )
    return "행사/프로그램";

  if (source.includes("비교과")) return "행사/프로그램";
...
function NoticeCard({
  ...
  item: NoticeItem;
  token: string;
  onAddCal: (item: NoticeItem) => Promise<void>;
}) {
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          >
            {resolveUrl(item.url || "", item.source) && (
              <a
                href={resolveUrl(item.url || "", item.source)}
                target="_blank"
                rel="noreferrer"
                style={{
                  display: "inline-flex",
      ...
      <div style={{ maxHeight: "65vh", overflowY: "auto" }}>
        {activeTab === "학과공지" && depts.length === 0 ? (
          ...
          displayItems.map((item, i) => (
            <NoticeCard key={i} item={item} token={token} onAddCal={onAddCal} />
          ))
        ) : (
          <div
            style={{
          ...
          >
            ...
            {cachedAt && !loading && (
              <div style={{ fontSize: "10px", color: "#CBD5E1" }}>
                {cachedAt.getHours()}:
                {cachedAt.getMinutes().toString().padStart(2, "0")} 기준
              </div>
            )}
          </div>
        </div>
      ...
      >
                      ...
                      >
                        {ds.map((d) => (
                          <Chip
                            key={d}
                            label={d}
                            selected={depts.includes(d)}
                            color="#1E40AF"
                            onClick={() => toggle(depts, d, setDepts)}
                          />
                        ))}
                      </div>
                    </div>
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                <button
                  ...
                  style={{
                    ...
                    fontSize: "13px",
                    fontWeight: 600,
                    background: loading ? "#E2E8F0" : "#1E40AF",
        ...
        {items.length > 0 && !loading && (
          <ResultView
            items={
              cats.length > 0
                ? items.filter((item) => cats.includes(categorize(item)))
                : items
            }
            depts={depts}
            token={token}
